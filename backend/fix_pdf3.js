...
const idx = f.indexOf('function generateMinimalPDF(visit) {');
if(idx !== -1) {
  const newCode = `
...
`;
  f = f.substring(0, idx) + newCode;
  fs.writeFileSync('routes/visits.js', f, 'utf8');
  console.log('Fixed generatePuppeteerPDF');
} else {
  console.log('generateMinimalPDF not found');
}
